...
public class CBC {
    ...
    public void Encrypt(String path, String key) {
        try {
            ...
            while ((i = (reader.read(bytes))) == 16) {
                ...
                ciphertext = camellia.cryptBlock(plaintext[0], plaintext[1], subkeys);
    ...
    public void Decrypt(String path, String key) {
        try {
            ...
            int i;
            long[] subkeys = camellia.keySchedule(key);
            subkeys = subkeys.length == 26 ? camellia.transformKeys128(subkeys) : camellia.transformKeys192_256(subkeys);
            while ((i = (reader.read(bytes))) == 16) {
                ...
                long[] plaintext = camellia.cryptBlock(ciphertext[0], ciphertext[1], subkeys);
                plaintext[0] ^= ex_ciphertext[0];
                plaintext[1] ^= ex_ciphertext[1];
                ...
                if (reader.available() == 0) {
                    ...
                    writer.flush();
                    ex_ciphertext[0] = ciphertext[0];
                    ex_ciphertext[1] = ciphertext[1];
                }
            }
        } catch (Exception e) {
            System.out.println("Error occurred");
